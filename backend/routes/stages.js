...
router.post('/input', requireAuth('admin', 'input'), upload.single('photo'), (req, res) => {
  const { input_material_id, input_qty, output_material_id, output_qty, source_company, notes } = req.body;
  ...
  const run = db.transaction(() => {
    ...
    const info = db.prepare(`INSERT INTO material_conversions
      (input_material_id, input_qty, output_material_id, output_qty, operator_id, source_company, photo_path, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(input_material_id, inQty, output_material_id, outQty, req.user.userId,
        source_company || '', req.file ? req.file.filename : null, notes || '');
...
router.post('/dispatch', requireAuth('admin', 'distribution'), upload.single('photo'), (req, res) => {
  const { product_id, qty, destination_person, destination_place, vehicle, notes,
    amount_collected, payment_method, paid } = req.body;
  const q = Number(qty);
...
router.get('/today-status', requireAuth('admin'), (req, res) => {
  ...
  res.json({
    today, yesterday,
    stages: [
      { role: 'input', label: 'Input (beads -> rolls)', today_count: input.today_count || 0, yesterday_count: input.yesterday_count || 0, latest: latest.input },
      { role: 'cutting', label: 'Cutting (rolls -> bags)', today_count: cutting.today_count || 0, yesterday_count: cutting.yesterday_count || 0, latest: latest.cutting },
      { role: 'distribution', label: 'Distribution', today_count: distribution.today_count || 0, yesterday_count: distribution.yesterday_count || 0, latest: latest.distribution },
